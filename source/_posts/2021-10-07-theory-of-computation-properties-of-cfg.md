...
**Pumping Lemma**: 令 $L$ 为一个 CFL，则存在常数 $n$ ，对于任意属于 $L$ 的 string $z$ ，若 $|z| \geqslant n$ ，则可以将 $z$ 写作 $uvwxy$ ，并且满足
...
**Reversal**: 如果 $L$ 是 CFL，则 $L^{R}$ 也是 CFL
...
**Inverse Homomorphism**: 证明思路类似证明正则语言对 inverse homomorphism 是封闭的。构造一个 PDA，每当读入一个输入 $a$ ，则将 $h(a)$ 放入一个 buffer 中，每次读取其中一个符号，用于模拟 PDA 的运行，直至读入结束，再读入下一个符号
...
**Emptiness**: 测试 CFL $L$ 是否为空只需要测试其开始符号 $S$ 是否为 generating 即可

CFL $L$ 非空 $\iff$ 其开始符号 $S$ 为 generating

**Membership**: 有一种算法利用 DP 的思想，若 string 的长度为 $n$ ，可以在 $O(n^{3})$ 的时间内确定这个 string 是否在给定的 CFL 中，即 CYK 算法。
...
**Infiniteness**: Infiniteness 的测试思想与正则语言相同，对于 pumping lemma 的常数 $n$ ，只需测试长度在 $[n, 2n-1]$ 的 string 是否都在 $L$ 中即可
